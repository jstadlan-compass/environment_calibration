...
all_scores <- scores %>% select(-colnames(scores)[grepl("Unnamed",colnames(scores))])
...
all_scores %>%
  ...
  theme(legend.position="bottom",
        legend.box = "vertical") +
  scale_linetype_manual(values=c(2)) +
  scale_fill_brewer(palette="Set2") -> bp2
...
for(i in unique(track_sets$best)){
  ...
  print(bp)
  ggsave(paste("output",exp,"performance",paste(i,"performance.png",sep="_"),sep='/'),bp,height = 8,width=8)
}
```
...
for(i in 1:nrow(pp))
{
  if(i==1){
    ...
    params <- read.csv(paste("output",exp,
                             paste('LF',pp$round[i],sep='_'),"translated_params.csv",sep='/')) %>%
              mutate(round=pp$round[i])
  } else {
    rbind.data.frame(params,
                    read.csv(paste("output",exp,
                             paste('LF',pp$round[i],sep='_'),"translated_params.csv",sep='/')) %>%
                    mutate(round=pp$round[i])) -> params
  }
}
...
param_levels <- unique(params$parameter)
...
if(length(ls_list)>1){  # Skip if post_calibration_analysis hasn't generated <site>-<metric>-LS files
  ...
  sensitivities %>%
    ...
    ggplot(aes(x=factor(id,levels=plevels,
                        labels=gsub("\n","",gsub("-\n"," ",params$parameter[plevels]))),
               y=paste(substr(metric,1,4),'.    ',substr(site,1,4),'.',sep=""))) +
    geom_tile(color="white",
              aes(fill=value)) +
  ...
  sensitivities %>%
    ...
    ggplot(aes(y=factor(id,levels=plevels,labels=gsub("\n","",gsub("-\n"," ",params$parameter[plevels]))),
               x=interaction(site,factor(metric,levels=c("prevalence","[asexual parasites]","[gametocytes]",
                                        "infectiousness","incidence"))))) +
    theme_minimal(base_size=16) +
    scale_fill_distiller(palette="RdGy",direction = 1) +
    theme(axis.text.x=element_text(angle=45,hjust=1)) +
    ...
    labs(fill=NULL) +
    #coord_fixed() +
    geom_point(aes(size=1/value,
                   alpha=1/value,
                   color=metric)) +
    scale_size_continuous(breaks = c(0.1,0.5,1,2,4,6,8))
...
```{r}
##### Severe Disease Plots #####

## Absolute Incidence ##
# Load selected simulation outputs
ddmp <- track_sets %>% filter(metric %in% c("severe_incidence"))
...
eirs <- read.csv("../simulation_inputs/monthly_eirs/eir_by_site.csv")

eirs %>% gather("Site","eir",-c("month")) %>% group_by(Site) %>% summarize(annual_eir=sum(eir)) -> eirs
...
inc_df %>%
  ...
  facet_wrap(~paste(gsub('.{5}$', '', Site)," (EIR = ",annual_eir,")",sep=""),ncol=2,scales="free_y") +
  geom_line(aes(group=interaction(param_set,round,Run_Number),
                color=paste("Round",round,"#",param_set),
                y=Severe.Incidence*1000)) +
  geom_point(data=inc_ref %>% left_join(eirs), aes(x=Age,y=absolute_severe_incidence*1000,
                               shape="Reference")) +
  labs(color=NULL,fill=NULL,shape=NULL) +
  theme_minimal(base_size=14) +
  theme(legend.position=c(1,0),
        legend.justification = c(1.1,0.25)) +
  ylab("Severe disease incidence (per 1,000)")
